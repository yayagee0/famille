...
<!-- Mobile menu button -->
<div
	class="sticky top-0 z-40 flex items-center gap-x-4 border-b px-4 py-4 shadow-sm sm:gap-x-6 sm:px-6 lg:hidden {currentTheme === 'neo'
		? 'border-white/20 bg-slate-900/95 backdrop-blur-md'
		: 'border-gray-200 bg-white'}"
>
	<button
		type="button"
		class="-m-2.5 p-2.5 transition-all duration-200 ease-in-out hover:scale-110 active:scale-95 lg:hidden {currentTheme === 'neo'
	...
	<div class="flex-1 text-sm leading-6 font-semibold {currentTheme === 'neo' ? 'neo-gradient-text' : 'text-gray-900'}">Family Hub</div>
	...
	{#if user && user.photoURL}
		<img
			class="h-8 w-8 rounded-full {currentTheme === 'neo' ? 'border border-white/30' : 'bg-gray-50'}"
			src={user.photoURL}
			alt={getDisplayName(user?.email, { nickname: user?.nickname })}
		/>
	{:else}
		<div class="h-8 w-8 rounded-full {currentTheme === 'neo' ? 'neo-glass border border-white/30' : 'bg-gray-300'}"></div>
	{/if}
</div>
	...
	<div class="relative z-50 lg:hidden">
		...
		<div class="fixed inset-0 flex">
			<div class="relative mr-16 flex w-full max-w-xs flex-1">
				...
				<div class="flex grow flex-col gap-y-5 overflow-y-auto px-6 pb-4 {currentTheme === 'neo'
					? 'neo-glass-nav backdrop-blur-md'
					: 'bg-white'}">
					<div class="flex h-16 shrink-0 items-center">
						<h1 class="text-xl font-bold {currentTheme === 'neo' ? 'neo-gradient-text' : 'text-gray-900'}">Family Hub</h1>
					</div>
					<nav class="flex flex-1 flex-col">
						<ul role="list" class="flex flex-1 flex-col gap-y-7">
							<li>
								<ul role="list" class="-mx-2 space-y-1">
									{#each navigation as item (item.href)}
										<li>
											<a
												href={item.href}
												class="group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold transition-all duration-200 {currentTheme === 'neo'
											...
											>
												{#if item.icon === 'home'}
													<Home class="h-6 w-6 shrink-0 transition-colors duration-200 {currentTheme === 'neo' && $page.url.pathname === item.href ? 'text-cyan-400' : ''}" />
												{:else if item.icon === 'rss'}
													<Rss class="h-6 w-6 shrink-0 transition-colors duration-200 {currentTheme === 'neo' && $page.url.pathname === item.href ? 'text-cyan-400' : ''}" />
												{:else if item.icon === 'moon'}
													<Moon class="h-6 w-6 shrink-0 transition-colors duration-200 {currentTheme === 'neo' && $page.url.pathname === item.href ? 'text-cyan-400' : ''}" />
												{:else if item.icon === 'gamepad'}
													<Gamepad2 class="h-6 w-6 shrink-0 transition-colors duration-200 {currentTheme === 'neo' && $page.url.pathname === item.href ? 'text-cyan-400' : ''}" />
												{:else if item.icon === 'user'}
													<User class="h-6 w-6 shrink-0 transition-colors duration-200 {currentTheme === 'neo' && $page.url.pathname === item.href ? 'text-cyan-400' : ''}" />
												{/if}
												{item.name}
											</a>
										</li>
							...
							<li class="mt-auto space-y-4">
								...
								{#if user}
									<div
										class="flex items-center gap-x-4 px-2 py-3 text-sm leading-6 font-semibold {currentTheme === 'neo' ? 'text-slate-200' : 'text-gray-900'}"
									>
										{#if user.photoURL}
											<img
												class="h-8 w-8 rounded-full {currentTheme === 'neo' ? 'border border-white/30' : 'bg-gray-50'}"
									...
									<button
										onclick={handleSignOut}
										class="group flex w-full gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold transition-all duration-200 ease-in-out hover:scale-105 active:scale-95 {currentTheme === 'neo'
											? 'text-slate-300 hover:text-white neo-glass hover:bg-white/10'
											: 'text-gray-700 hover:bg-gray-50 hover:text-indigo-600'}"
									>
										<LogOut
