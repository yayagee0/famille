...
test.describe('SvelteKit Application', () => {
	...
	test('should handle client-side navigation', async ({ page }) => {
		...
		const criticalErrors = (page.errors || []).filter(
			(error) =>
				!error.includes('Firebase') && !error.includes('auth') && !error.includes('Network')
		);
	...
	test('should load CSS and fonts without errors', async ({ page }) => {
		...
		const unexpectedFailures = failedRequests.filter(
			(url) =>
				!url.includes('googleapis.com') && !url.includes('firebase') && !url.includes('gstatic.com')
		);
